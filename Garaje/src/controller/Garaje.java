...
public class Garaje {
    ...
    public void areglarCoche(String matricula){
        ...
        if (coche  != null){
            ...
            coche= null;
            System.out.println("Coche esta");
        } else {
            System.out.println("no encontramos el coche");
        }
    }
    private void eliminarCoche(String matricula){
        for (int i = 0; i < listaCoches.length; i++) {
            if (listaCoches[0][0].toString().equals(matricula)){
                listaCoches[i] = new Object[listaCoches[i].length];
                break;
            }
        }
    }
    private double getAcumulado(){
        return acumulado;
    }
